					 ...
					 radio.addEventListener('change', function () {
						 ...
						 if (self.value == 'auto') {
							 ...
							 media.addEventListener('timeupdate', function (e) {
								 ...
								 var newIndex = parseInt(netspeed / 0.5)
								 var newQuality = Array.from(qualityMap.keys())[qualityMap.size - newIndex - 1]
								 if (media.getSrc() === qualityMap.get(newQuality)[0].src) return
								 t.updateVideoSource(media, qualityMap, newQuality);
								 media.setSrc(qualityMap.get(newQuality)[0].src);
								 media.load();
								 media.setCurrentTime(currentTime);
								 media.dispatchEvent(mejs.Utils.createEvent('seeking', media));
								//  console.log('here!! ', netspeed, newQuality)
								 if (!paused) {
									 media.play();
								 }
